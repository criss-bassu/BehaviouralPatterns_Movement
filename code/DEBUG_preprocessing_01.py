...
class WeeklyAccelerometryDataset(Dataset):
    def __init__(self, descriptors, ClinicalCovariates, OutcomeTargets, mask, df, split, augment = False):
        print(f"\nCreating WeeklyAccelerometryDataset for split: {split}")
        print(f"augment: {augment}")

        # Indices of the rows belonging to the split selected
        self.indices = df.index[df["split"] == split].to_numpy()
        print(f"Number of rows in split '{split}': {len(self.indices)}")
        print(f"First indices in split '{split}': {self.indices[:10]}")
        ...
        print(f"descriptors tensor shape for '{split}': {self.descriptors.shape}")
        ...
        print(f"ClinicalCovariates tensor shape for '{split}': {self.ClinicalCovariates.shape}")
        ...
        print(f"OutcomeTargets tensor shape for '{split}': {self.OutcomeTargets.shape}")

        # Data tensor with the mask values for the selected rows
        self.mask = torch.tensor(mask[self.indices], dtype = torch.float32)
        print(f"mask tensor shape for '{split}': {self.mask.shape}")
        ...
        print(f"First participant_id values for '{split}': {self.participant_id[:10]}")
    ...
    def __getitem__(self, idx):
        ...
        if idx == 0:
            ...
            print(f"Original descriptor shape: {d.shape}")
            print(f"Covariate shape: {self.ClinicalCovariates[idx].shape}")
            print(f"Target values: {self.OutcomeTargets[idx]}")
            print(f"Mask values: {self.mask[idx]}")
            print(f"Participant ID: {self.participant_id[idx]}")

        # if augment == TRUE
        if self.augment:
            if idx == 0:
                print("Applying data augmentation to idx = 0")
            ...
            if idx == 0:
                print(f"Descriptor shape after augmentation: {d.shape}")
                print(f"Number of visible hours: {visible_hour.sum().item()} / {visible_hour.shape[0]}")
...
def load_data(
    ...
    print("\nStarting load_data()")
    print(f"descriptors_path: {descriptors_path}")
    ...
    print(f"NaN values in descriptors before scaling: {np.isnan(descriptors).sum()}")
    print("Missing values per target before preprocessing:")
    ...
    print(f"First participants: {participants.head(10).tolist()}")

    strat_col = BINARY[0] if BINARY else None # DMT2
    print(f"strat_col: {strat_col}")
    ...
    if participant_strat is not None:
        print("participant_strat value counts:")
    ...
    print(f"Number of train participants: {len(train_p)}")
    print(f"Number of temp participants: {len(temp_p)}")
    ...
    print(f"Number of validation participants: {len(val_p)}")
    print(f"Number of test participants: {len(test_p)}")
    ...
    print("\nSplit distribution by unique participants:")
    ...
    print(f"\ntrain_idx shape: {train_idx.shape}")
    print(f"First train_idx values: {train_idx[:10]}")
    ...
    print(f"\ndescriptors_train_flat shape: {descriptors_train_flat.shape}")

    # Compute the mean of each descriptor
    descriptors_mean = descriptors_train_flat.mean(axis = 0)
    print(f"descriptors_mean shape: {descriptors_mean.shape}")
    print(f"First descriptor means: {descriptors_mean[:10]}")
    ...
    print(f"descriptors_std shape: {descriptors_std.shape}")
    print(f"First descriptor stds: {descriptors_std[:10]}")

    # Descriptors normalized (NaN -> 0)
    descriptors_scaled = np.nan_to_num((descriptors - descriptors_mean) / descriptors_std, nan = 0.0)

    print(f"descriptors_scaled shape: {descriptors_scaled.shape}")
    print(f"NaN values in descriptors_scaled: {np.isnan(descriptors_scaled).sum()}")
    print(f"Min value in descriptors_scaled: {descriptors_scaled.min()}")
    print(f"Max value in descriptors_scaled: {descriptors_scaled.max()}")
    ...
    print(f"NaN values in cov: {np.isnan(cov).sum()}")
    print(f"First cov row: {cov[0] if len(cov) > 0 else 'EMPTY'}")
    ...
    print(f"\ntarget_raw shape: {target_raw.shape}")
    print(f"First target_raw row: {target_raw[0] if len(target_raw) > 0 else 'EMPTY'}")

    # Whether the targets are valid (1) or missing (0)
    mask = (~df[TARGET_COLS].isna()).to_numpy(dtype = "float32")
    print(f"mask shape: {mask.shape}")
    print(f"First mask row: {mask[0] if len(mask) > 0 else 'EMPTY'}")

    print("Valid target counts:")
    for i, col in enumerate(TARGET_COLS):
        print(f"{col}: {int(mask[:, i].sum())} valid values / {mask.shape[0]} rows")
    ...
    print(f"numeric_targets_standardised shape: {numeric_targets_standardised.shape}")
    print(f"First numeric_targets_standardised row: {numeric_targets_standardised[0] if len(numeric_targets_standardised) > 0 else 'EMPTY'}")
    print(f"NaN values in numeric_targets_standardised: {np.isnan(numeric_targets_standardised).sum()}")
    ...
    print("\nload_data() finished successfully")
...
def verify_no_leakage(df):
    ...
    for split_name, participant_set in splits.items():
        print(f"{split_name}: {len(participant_set)} unique participants")
    ...
    print("No leakage detected between train, validation and test")
...
def normalize_numeric_targets(target_raw, mask, train_idx):
    print("\nRunning normalize_numeric_targets()")
    ...
    for i, col in enumerate(TARGET_COLS):
        ...
        if len(valid_target) > 0:
            ...
            print(f"Mean for {col}: {target_mean[i]}")
            print(f"Std for {col}: {target_std[i]}")
        else:
            print(f"No valid training values found for {col}. Mean remains 0 and std remains 1.")
    ...
    for i, col in enumerate(TARGET_COLS):
        if col not in BINARY:
            print(f"Standardizing target: {col}")
    ...
    print("\nnormalize_numeric_targets() finished")
    print(f"target_mean: {target_mean}")
    print(f"target_std: {target_std}")
    print(f"result shape: {result.shape}")
    print(f"NaN values in result: {np.isnan(result).sum()}")
